...
import { GET_ALL_LOCATION, SCHOOL_SIZE, SCHOOL_lIST_PAGINATION } from "../../GraphQl/Queries";
...
export const ManageSchool = () => {
    ...
    useEffect(()=>{
     ...
     fetch().catch(e=>console.log('error fetching school '+e));
     fetchLocation().catch(e=>console.log('error fetching Location '+e));
    },[data,sortBy,pageNumber,pageSize,sectorId]);
    const nextPage = () => {
    ...
    const add_school_Modal = <div className="modal fade" id="school_Modal" data-bs-backdrop="static" data-bs-keyboard="false" aria-labelledby="school_ModalLabel" aria-hidden="true">
        <div className="modal-dialog">
            <div className="modal-content">
                <div className="modal-header">
                    <h5 className="modal-title" id="school_ModalLabel">Add new hospital</h5>
                ...
                <div className="modal-footer">
                    ...
                    <button type="button" className="btn btn-primary" data-bs-dismiss="modal" onClick={createHospital}>Save</button>
                </div>
            </div>
        </div>
    </div>;
    return (
        <AdminNavigationBar>
           <div className="fw-bold  display-6  bg-primary-subtle mt-2 p-3">Manage School</div>
            <section className="col-sm-4 ">
                <div className="row m-auto bg-primary-subtle">
                    ...
                    <div className="card col-sm-6 d-flex bg-primary-subtle justify-content-center border-0">
                        ...
                        {data&& <>{data.schoolListPagination.size}</>}
                        </span>
                        </div>
                    </div>
                </div>
            ...
            <div className="py-4">
                <i className="bi bi-plus-circle-fill fs-2" data-bs-toggle="modal" data-bs-target="#school_Modal" onClick={()=>setPageSize(11)}></i>
            </div>
            {
                loading ?
                    ...
                    <div className='row m-auto col-12 '>
                        {
                            school.length > 0 && <div className='mt-2 mb-2'>
                                <span className="text-white-emphasis text-bg-primary p-1 px-2 fw-bold">
                                Page {data.schoolListPagination.pageNumber + 1} 
                                </span>
                                        ...
                                        <div className="col-sm-4">
                                            <div className="card border-0">
                                                <div className="card-body">
                                                   ...
                                                   {data.location!=null&&<p className="card-text">
                                                        <i className='bi bi-geo-fill'></i>{data.location.location.location.name}/ {data.location.location.name}/{data.location.name}
                                                    </p>}
                                                </div>
                                            </div>
                                        </div>
